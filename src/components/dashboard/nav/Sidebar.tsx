...
const Sidebar = ({ userType, isCollapsed}) => {
  ...
  return (
    <aside className={`${isCollapsed ? 'w-20' : 'w-72'} bg-gradient-to-b from-slate-900 to-slate-800 text-white h-screen flex flex-col transition-all duration-300 ease-in-out relative shadow-2xl`}>
      {/* Header */}
          ...
          {!isCollapsed && (
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">{userType === 'admin' ? 'Admin Portal' : 'User Portal'}</p>
              </div>
            </div>
          )}
          {isCollapsed && (
